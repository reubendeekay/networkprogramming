...
int main(int argc, char **argv)
{
    ...
    // SOCK_STREAM: The type of socket to create in this case a stream socket because it is a connection oriented socket
    ...
    // Listen for incoming connections. 5:Maximum number of pending connections
    if (listen(sock, 5) < 0)
    {
        perror("listen");
        exit(EXIT_FAILURE);
    ...
    int k = recv(j, client_data, sizeof(client_data), 0);
    if (k < 0)
    {
        perror("recv");
        exit(EXIT_FAILURE);
    ...
    // token1 = strtok(client_data_copy,",");
    // Extract the serial number(1st token) and store it in a variable called ser
    // ser = atoi(token1);
    // Extract the registration number and store it in a variable called reg
    // token2 = strtok(NULL,",");
    // strcpy(reg, token2);
    ...
    // Send reply to the client about success in saving the records
